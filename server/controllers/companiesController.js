...
export const getCompanyProposal = async (req, res) => {
  const companyId = Number(req.params.id);
  if (!Number.isInteger(companyId) || companyId <= 0) {
    return res.status(400).json({ message: 'Identificador da empresa inválido.' });
  }

  const [rows] = await pool.query(
    `SELECT id, fantasy_name, corporate_name, cnpj, ie, address, zip, city, state, phone, cel, whatsapp, email,
            instagram, business_activity, foundation_date, employees_qty, sector, accounting_office, referred_by, note,
            plan_type, value, commission_rate, commission_exempt, due_date, signature_url, status, created_at, updated_at
       FROM companies
      WHERE id = ?
      LIMIT 1`,
    [companyId]
