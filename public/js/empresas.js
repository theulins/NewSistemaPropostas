...
async function generateCompanyPdf(company, { targetWindow } = {}) {
...
async function handleExportCompanyPdf(id, button) {
  ...
  if (!previewWindow) {
    showError('Não foi possível abrir o PDF. Verifique o bloqueio de pop-ups.');
    return;
  }
  try {
    if (button) {
    ...
    const company = await fetchCompanyDetails(id);
    await generateCompanyPdf(company, { targetWindow: previewWindow });
  } catch (error) {
